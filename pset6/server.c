...
char* indexes(const char* path)
{
    char* indxphp = "/index.php";
    char* php_path;
    if ( (php_path = malloc(strlen(path)+strlen(indxphp)+1)) != NULL)
    {
        php_path[0] = '\0';   // ensures the memory is an empty string
        strcat(php_path,path);
        strcat(php_path,indxphp);
    }
    ...
    char* indxhtml = "/index.html";
    char* html_path;
    if ( (html_path = malloc(strlen(path)+strlen(indxhtml)+1)) != NULL)
    {
        html_path[0] = '\0';   // ensures the memory is an empty string
        strcat(html_path,path);
        strcat(html_path,indxhtml);
    }
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // Determine Method
    int method_lenght = firstSP - 1;
    char* method = malloc(method_lenght + 1);
    strncpy (method, line, method_lenght);
    method[method_lenght] = '\0';

    // Determine Request Target
    int request_target_lenght = secondSP - firstSP - 1;
    char* request_target = malloc(request_target_lenght + 1);
    strncpy (request_target, &line[firstSP], request_target_lenght);
    request_target[request_target_lenght] = '\0';
    
    int absolute_path_lenght = q - firstSP - 1;
    char* absolute_path = malloc(absolute_path_lenght + 1);
    strncpy (absolute_path, &line[firstSP], absolute_path_lenght);
    absolute_path[absolute_path_lenght] = '\0';
    
    int realquery_lenght = secondSP - q;
    char* realquery = malloc(realquery_lenght + 1);
    if (realquery_lenght > 0)
    {
        strncpy (realquery, &line[q], realquery_lenght);
        realquery[realquery_lenght] = '\0';
    }
    else
    {
        realquery[realquery_lenght] = '\0';
    ...
    int HTTP_version_lenght = strlen(line) - secondSP;
    char* HTTP_version = malloc(HTTP_version_lenght + 1);
    strncpy (HTTP_version, &line[secondSP], HTTP_version_lenght);
    HTTP_version[HTTP_version_lenght] = '\0';
    ...
    else if (strncmp(&request_target[0], "/", 1) != 0)
    {
        error(501);
        return false;
    }
    
    else if (strstr(&request_target[0], "\"") != NULL)
    {
        error(400);
        return false;
    }
    
    else if (strncmp(&HTTP_version[0], "HTTP/1.1", 8) != 0)
    {
        error(505);
        return false;
    }
    ...
    printf("this is the righteous path ====> %s\n", abs_path);
